...
const {
  createCarReservationHistory: createHistory,
} = require("./car_reservation_history.controller");

const getCarReservations = async (req, res) => {
  const { uuid, car_id, user_uuid, driver_uuid, sort, is_active, is_delete } =
    req.query;
...
const updateCarReservation = async (req, res) => {
  ...
  try {
    ...
    if (car_uuid && car_uuid !== null && car_uuid !== "") {
      const car = await carModel.findOne({
        where: {
          uuid: car_uuid,
        },
    ...
    carReservation.start_location =
      start_location || carReservation.start_location;
    carReservation.finish_location =
      finish_location || carReservation.finish_location;
