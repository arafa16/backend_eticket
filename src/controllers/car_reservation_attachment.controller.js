...
const createAttachment = async (req, res) => {
  ...
  const findData = await carReservationModel.findOne({
    where: {
      uuid,
    },
  });
  ...
  const file_link = `/attachment/car_reservation/${file_name}`;
  ...
  file.mv(`./public/attachment/car_reservation/${file_name}`, async (err) => {
    if (err) return res.status(500).json({ message: err.message });
    try {
      await carReservationAttachmentModel.create({
        car_reservation_id: findData.id,
        name: file.name,
        file_name,
        file_link,
      });
...
const deleteAttachment = async (req, res) => {
  ...
  const findData = await carReservationAttachmentModel.findOne({
    where: {
      uuid,
    },
  });
